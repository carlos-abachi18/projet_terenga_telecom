...
# def joind ():
data_pop = pd.read_csv("./projet_data/telecom_zipcode_population.csv")
data_pop.rename(columns={'Zip Code':'Zip_Code'},inplace=True)
data_pop = pd.merge(customers_data, data_pop, on='Zip_Code', how='left')

population = data_pop[['Population',"City"]]
...
df= pd.DataFrame(df)
print(df)
big_city_data = data_pop[['Population','City','Latitude','Longitude',]].head(30)
...
cities_data = {
    "City": ["Los Angeles", "San Diego", "San Jose", "Sacramento", "San Francisco", "Fresno", "Long Beach", "Oakland", "Escondido", "Stockton", "Fallbrook", "Glendale", "Bakersfield", "Temecula", "Berkeley",
             "Riverside", "Whittier", "Pasadena", "Anaheim", "Irvine", "Modesto", "San Bernardino", "Santa Barbara", "Inglewood", "Santa Monica", "Burbank", "Torrance", "Santa Ana", "Santa Rosa",
             "Chula Vista", "Orange"],
    "Population": [3900, 1400, 1000, 525, 875, 550, 460, 440, 151, 320, 35, 196, 410, 114, 126, 330, 85, 140, 350, 310, 220, 220, 90, 107, 90, 103, 145, 310, 180, 280, 140]
}
...
City_data = pd.merge(city, city_churn, on="City", how='left')
City_data['Nombre_de_Désabonné'] = City_data['Nombre_de_Désabonné'].fillna(0).astype(int)
City_data = pd.merge(City_data,df_cities,on='City',how='inner')
City_data['taux_de_couverture']= (City_data['Nombre_de_Client'] / City_data['Population'])*100
City_data